...
export default async function LocaleLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return <div lang={selected} dir={selected === 'ar' ? 'rtl' : 'ltr'} className="dc-shell"><StorageMigration/><SiteHeader locale={selected}/><GameStatusBar locale={selected}/><main>{children}</main><SiteFooter locale={selected}/><MobileGameNav locale={selected}/></div>;
}
