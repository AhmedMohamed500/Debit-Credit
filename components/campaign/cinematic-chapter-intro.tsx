...
export function CinematicChapterIntro({chapter,locale,onComplete,onSkip}:Props){
 ...
 return <section className="fd-cinema-player fd-entry-gate" dir={ar?'rtl':'ltr'} aria-label={say('Chapter cinematic entry','مدخل الفصل السينمائي')}>
  ...
  <header className="fd-cinema-hud"><span><b>Debit & Credit™</b><small>by Money Coder</small></span><div><i>{say('ACCOUNTING CINEMATIC UNIVERSE','عالم المحاسبة السينمائي')}</i><strong>{chapter.chapterTitle[locale]}</strong></div><Link className="fd-cinema-manual" href={`/${locale}/account-guide?return=first-day`} target="_blank" rel="noopener noreferrer"><BookOpen/>{say('Nature of Accounts','طبيعة الحسابات')}</Link><Link href={`/${ar?'en':'ar'}`}><Languages/>{ar?'EN':'AR'}</Link></header>
