...
export function DetectiveHome({ locale }: { locale: Locale }) {
  ...
  return <div className="detective-home">
    <section className="detective-hero"><div className="detective-container detective-hero-grid"><div>
      <Link className="detective-academy-back" href={`/${locale}/learning-map`}>{ar ? <ArrowRight size={16}/> : <ArrowLeft size={16}/>}<GraduationCap size={16}/>{ar ? "خريطة التعلم / اكتشاف الأخطاء" : "Learning Map / Error Detection"}</Link>
      <span className="detective-kicker"><Search size={17}/>Debit & Credit Accounting Detective</span>
