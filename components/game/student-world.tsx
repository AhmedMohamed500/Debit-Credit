...
export function StudentWorld({children}:{children:ReactNode}) {
  ...
  const {state}=useGame();
 const modern=/^\/(ar|en)\/(game|academy|account-guide|leaderboard|career|career-league|employers|journal|ledger|trial-balance|financial-statements|challenges|profile)(\/|$)/.test(pathname);
