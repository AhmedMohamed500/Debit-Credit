...
export function MissionsHome({ locale }: { locale: Locale }) {
  ...
  return <div className="missions-page">
    <section className="missions-hero"><div className="missions-container missions-hero-grid"><div>
      <span className="missions-kicker"><Sparkles size={17}/>{ar ? "Debit & Credit Missions" : "Debit & Credit Missions"}</span>
    ...
    <section className="missions-container missions-catalog"><div className="mission-section-title"><span>{ar ? "ابدأ التطبيق فورًا" : "Start practicing now"}</span><h2>{ar ? "خمس حالات قصيرة من الشغل الحقيقي" : "Five short cases from real work"}</h2><p>{ar ? "مفيش درس طويل. اختار قرارك وشوف تأثيره على الشركة والقيد." : "No long lessons. Make a decision and see its impact on the business and entry."}</p></div>
      ...
      <aside className="mission-detective-cta"><div><span>{ar ? "المرحلة التالية في رحلتك" : "NEXT MODE IN YOUR JOURNEY"}</span><h3>{ar ? "جاهز تجرب قضية حقيقية؟" : "Ready to investigate a real case?"}</h3><p>{ar ? "افتح ملفًا كاملًا، راجع المستندات والحركات، ثم ابنِ استنتاجك بنفسك." : "Open a complete file, inspect documents and movements, then build your own conclusion."}</p></div><Link className="mission-primary" href={`/${locale}/detective`}>{ar ? "افتح المحقق المحاسبي" : "Open Accounting Detective"}{ar ? <ArrowLeft/> : <ArrowRight/>}</Link></aside>
      <aside className="mission-detective-cta"><div><span>DEBIT & CREDIT MONEY FLOW LAB</span><h3>{ar ? "شوف القيمة وهي بتتحرك" : "Watch value move"}</h3><p>{ar ? "ابدأ بزاد ونقص، وبعد فهم الحركة اكتشف بنفسك ليه الحساب مدين أو دائن." : "Start with increase and decrease, then discover debit and credit from the movement."}</p></div><Link className="mission-primary" href={`/${locale}/money-flow`}>{ar ? "افتح حرّك الفلوس" : "Open Money Flow Lab"}{ar ? <ArrowLeft/> : <ArrowRight/>}</Link></aside>
    </section>
  </div>;
}
