...
describe("Money Flow training isolation and progress",()=>{
 ...
 it("keeps domain evidence and feeds unified progress",()=>{recordMoneyFlowResult(result());expect(localStorage.length).toBe(2);expect(localStorage.getItem(MONEY_FLOW_STORAGE_KEY)).toBeTruthy();expect(localStorage.getItem(PLAYER_STORAGE_KEY)).toBeTruthy();});
